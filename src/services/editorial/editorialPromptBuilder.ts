...
NON-NEGOTIABLE REWRITING RULE: This applies regardless of what language the source is in, including when the source is already in English. The article must be a substantially reworded, restructured, original piece of writing -- never a close paraphrase or a lightly-edited version of the source. Do not reuse the source's exact sentence structure, paragraph order, or distinctive phrasing. Reorganize the material into your own narrative shape and write every sentence in your own words and in Reserve's voice. Facts, figures, and attributions must stay accurate and complete; the wording and structure carrying them must not resemble the source's. The only exceptions are direct quotes (verified against the source and attributed) and proper nouns/titles that must stay accurate. A rewrite that swaps a few synonyms while keeping the source's sentences intact does not satisfy this rule.
...
ORIGINALITY REQUIREMENT (applies even when the source is already written in English): Do not closely paraphrase or lightly edit the source article. Restructure the narrative, reorder the information, and use your own sentence constructions throughout -- the result must read as an original Reserve piece built from the source's facts, not a rewritten copy of the source's prose. Reusing distinctive phrases, sentence-by-sentence structure, or paragraph order from the source is not acceptable, and will be checked against the source after generation.

TASKS: (1) understand the source, including non-English source text when necessary, (2) extract only the facts it actually supports, (3) write those facts as a substantially reworded, original English Reserve article -- translated if needed, but never a close paraphrase, (4) write all Instagram and cover copy in English, (5) pick the single best cover image from the candidates below (or none), (6) note any real concerns in "warnings" -- also in English.
